    ...
    def _make_layers(self, cfg, channels=1):
        ...
        for x in cfg:
            ...
            if x == 'D':
                ...
                layers += [nn.Conv2d(in_channels, x, kernel_size=3, padding=1, bias=False)]  ##initial in_channels=3

                layers += [nn.BatchNorm2d(x,affine=True),nn.ReLU(inplace=True)]  ##initial in_channels=3

                test_batch = nn.BatchNorm2d(x)
                #print("mean here", test_batch.running_mean)
                #print("variance here", test_batch.running_var)
                #print("gamma here", test_batch.weight)
                #print("beta here", test_batch.bias)
    ...
    def _make_layers(self, cfg, channels=1):
        ...
        for x in cfg:
            if x == 'D':
                ...
                layers += [nn.Conv2d(in_channels, x, kernel_size=3, padding=1, bias=False)] ##padding=1 == padding ='same'
                layers += [nn.BatchNorm2d(x,affine=True),nn.ReLU(inplace=True)]  ##initial in_channels=3


                #print(" child mean here",test_batch.running_mean)
                #print("child variance here",test_batch.running_var)
                #print("child gamma here",test_batch.weight)
                #print("child beta here",test_batch.bias)
...
def train(model, model_three, optimizer, loss_fn, trainloader, valloader, device, maxi):
    ...
    for epoch in range(30):
        ...
        for batch_num, (data, target) in enumerate(trainloader):
            data = data.to(device)
            target = target.to(device)
            #print("target here",target) ##original input data
            ...
            #print("indices here", indices.shape)
            ...
            #print("three_target is here",three_target)##only 1,2,3 label

            three_target1 = three_target.clone().to(device)
            #print("three_target1 here before = ", three_target1)

            three_target1[(three_target1 == 1).nonzero()] = 10
            three_target1[(three_target1 == 2).nonzero()] = 11
            three_target1[(three_target1 == 3).nonzero()] = 12
        ...
        print("Epoch: ", epoch + 1, "\nTrain: ", model_three.acc / num_three)
         ...
         #   print("checkpoint saved")
          #  max = model_three.acc / num_three
           # torch.save(model_three.state_dict(), "CWRU_1.pth")
        ...
        #print("test_Data_three", num_three)
...
class MyDatasetSNR(Dataset):
    def __init__(self, txt):
        ...
        for line in fh:
            allImage = line.strip('\n')
            if 'FE' in allImage:
                path = '/home/shun/Desktop/Research/MNN-Tree/CWRU/CWRU2D/CWRU_data_image9bit_SNR10/'
                path_new = path + line.split('/')[-2] + '/' + line.split('/')[-1].split('.')[-2] + '.tif'
                input_img = io.imread(path_new)
                input_img_resize = cv2.resize(input_img, (64, 64))
                input_img_resize = np.resize(input_img_resize, (1, 64, 64))
...
if __name__ == "__main__":
    ...
    train_datasetSNR = MyDatasetSNR(txt='/home/shun/Desktop/Research/MNN-Tree/CWRU/CWRU2D/test.txt')


    train_concat = torch.utils.data.ConcatDataset([train_dataset,train_datasetSNR])




    val_dataset = MyDataset(txt='/home/shun/Desktop/Research/MNN-Tree/CWRU/CWRU2D/test.txt')
    #val_dataset1 = MyDataset(txt='/home/shun/Desktop/Research/MNN-Tree/CWRU/CWRU2D/test.txt')

    #val_dataset2 = torch.utils.data.ConcatDataset([val_dataset,val_dataset1])

    #val_datasetSNR10 = torch.utils.data.ConcatDataset([val_dataset,test_datasetSNR])



    train_loader = torch.utils.data.DataLoader(train_concat,
    ...
    model_three = model_3().to(device)

    #model_three.load_state_dict(torch.load('./CWRU_1.pth'))

    #print("model_here 1",model_three.features[1].weight)
    #print("model_here 2",model_three.features[1].bias)
    #print("model_here 3",model_three.features[1].running_mean)

    #print("model_here 4",model_three.features[5].weight)
    #print("model_here 5",model_three.features[5].bias)
    #print("model_here 6",model_three.features[5].running_mean)
    ...
    # print("stop here now")
    count_parameters(model_three)
    max = train(model, model_three, optimizer, loss_fn, train_loader, val_loader, device, 0)
    ...
    for i in range(4):
        ...
        optimizer = torch.optim.Adam(model_three.parameters(), lr=learning_rate, weight_decay=5e-4)
        loss_fn = nn.CrossEntropyLoss()
        max = train(model, model_three, optimizer, loss_fn, train_loader, val_loader, device, max)
        #print("model_here 1", model_three.features[1].weight)
        #print("model_here 2", model_three.features[1].bias)
        #print("model_here 3", model_three.features[1].running_mean)

        #print("model_here 3", model_three.features[5].weight)
        #print("model_here 4", model_three.features[5].bias)
        #print("model_here 6", model_three.features[5].running_mean)
